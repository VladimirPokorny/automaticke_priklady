...
class TitleWithIcon(QtWidgets.QWidget):
    def __init__(self, name, icon_file):
        ...
        self.setStyleSheet("background-color:#3f3e3e;"
                           "font-size:18;"
                           "font-weight:bold;"
                           "margin:0px;"
                           "padding-left:5px;")
...
class Toolbar(QtWidgets.QWidget):
    def __init__(self, name: str, icon, expand_type="left"):
        ...
        self.setMaximumWidth(config.panel_size * 2)
    ...
    def expand(self, value):
        if self.expand_type == "right":
            if value:
                self.setFixedWidth(config.panel_size * 2 - self.slider_width)
                self.move(self.pos().x() - config.panel_size, self.pos().y())
            else:
                self.setFixedWidth(config.panel_size - self.slider_width)
                self.move(self.pos().x() + config.panel_size, self.pos().y())
        else:
            if value:
                self.setFixedWidth(config.panel_size * 2 - self.slider_width)
